...
?>
    ...
    <main role="main" class="ml-sm-auto col-9 pt-3 px-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2">
            <h1 class="h2">Конкуренты "<?php echo $campaign['name']; ?>"</h1>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><i class="far fa-folder-open"></i> Конкуренты</li>
            </ol>
        </nav>
        <!-- /Временные крошки -->
        ...
        if (isset($_POST['submit']) && $errors == FALSE) {
            echo "<div class='alert alert-success' role='alert'>Конкурент добавлен!</div>";
        }
        ?>

        <form action="" method="post">
            <div class="d-flex justify-content-between align-items-center pb-2" id="hideForm" style="display: none">
                <input type="text" class="form-control mr-4" name="competitor_url" placeholder="Укажите ссылку на любую страницу конкурента">
                <div class="btn-toolbar mb-2 mb-md-0"><button type="submit" name="submit" class="btn btn-primary">Добавить</button></div>
            </div>
        </form>


        <?php if ($countCompetitors == 0) {
            echo "<div class='alert alert-warning mt-4' role='alert'>У Кампании нет конкурентов!</div>";
        } else {
            echo $grid;
        } ?>
